...
import { ArrowLeft, ArrowRight, User, Building2, ChefHat, GraduationCap } from 'lucide-react';
...
export function BasicDetails({ role, onBack, onComplete }: BasicDetailsProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (role === 'manager') {
      ...
      if (memberName.trim() && education.trim()) {
        const details = { name: memberName, education };
        saveUserToDatabase(details);
        onComplete(details);
      }
    }
  ...
  const isValid = role === 'manager' 
    ...
    : memberName.trim() && education.trim();
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <h1 className="text-white" style={{ fontSize: '1.25rem', fontWeight: '700', letterSpacing: '-0.02em' }}>
              {role === 'manager' ? 'Mess Details' : 'Basic Details'}
            </h1>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    id="education"
                    type="text"
                    placeholder="e.g., BTech 2nd Year, MSc 1st Year"
                    value={education}
