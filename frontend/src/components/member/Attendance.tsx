...
export function Attendance({ onNavigate }: AttendanceProps) {
  ...
  const loadAttendance = async () => {
    try {
      ...
      if (!currentUser.messId || !currentUser.id) {
      ...
      const records = await attendanceService.getMemberAttendance(
        currentUser.messId,
        currentUser.id,
        startDate,
        endDate
      ...
      // Group by date and aggregate status (if multiple meals per day)
      ...
      records.forEach((record: any) => {
        ...
        // If present in any meal, mark as present; otherwise check for leave
  ...
  const leaveDays = attendanceData.filter(d => d.status === 'on-leave').length;
                    ...
                    <div
                      ...
                      style={{
                        background: status === 'present' ? 'rgba(11, 128, 67, 0.1)' :
                                  ...
                                  status === 'on-leave' ? 'rgba(234, 179, 8, 0.1)' :
                                  'transparent',
